...
let transform_name_encode' (n: name): Tac name
  = nameCurMod' n (fun x -> x ^ "_generic_encode_chainable")
let transform_name_encode (n: name): Tac name
...
let generateEncodeGeneric_for_inductiveSumup
  ...
  = let {iVars; iName; iCons} = s in
    ...
    let body = generateEncodeGeneric_term_for_inductiveSumup s in
    let mk (fName: _ -> Tac _) = Sg_Let false (pack_fv (fName s.iName)) [] (`_) in
    let sg
